...
def cleansing_df(df):
    ...
    common_months = set(df_2024['월']) & set(df_2025['월'])
    max_common_month = max(common_months) if common_months else 1

    df_2024_common = df_2024[df_2024['월'] < max_common_month]
    df_2025_common = df_2025[df_2025['월'] < max_common_month]
...
def viz_rate(df_2024_common, df_2025_common, selected_panel, pos, neg, df_diff_count, df_diff_rate):
    ...
    # 레이아웃 설정
    fig.update_layout(
        title_text=f"{selected_panel} 월간 이탈률 분석",
        showlegend=True,
        height=800,
        width=1400
    )
...
if not df.empty and panel_cos:
    ...
    if not df_2024_common.empty and not df_2025_common.empty:
        ...
        # 공통 컬럼들
        comparison_df['2024_연도-월'] = df_2024_reset['연도'].astype(str) + '-' + df_2024_reset['월'].astype(str) + '월'
        ...
        # 2025년 공통 컬럼들
        comparison_df['2025_연도-월'] = df_2025_reset['연도'].astype(str) + '-' + df_2025_reset['월'].astype(str) + '월'
        ...
        st.warning("⚠️ 비교할 수 있는 공통 월간 데이터가 없습니다.")
